...
export function EducatorInsightsPage(props: {
  ...
  const empty =
    metrics.length === 0 && insights.length === 0 && badges.length === 0;
    ...
    <div className="space-y-10">
      <header>
        ...
        <p className="mt-2 text-sm text-[var(--muted-foreground)]">
          Indicadores e ideas de mejora cuando tengamos analítica conectada.
        </p>
      </header>
      {empty ? (
        <p className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--card)] px-6 py-10 text-center text-sm text-[var(--muted-foreground)]">
          Aún no hay métricas agregadas para tu cuenta.
        </p>
      ) : (
        <>
